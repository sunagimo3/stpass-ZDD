...
//経由頂点を置くことでfalse枝刈りは早くなるがtrue枝刈りは遅くなる

//頂点数,辺数,経由点,始点,終点,グラフ情報を入力する
int ZDD(int V,int E,int s,int t,set<int> &setx,vector<pair<int,int>> &G,vector<vector<int>> &g,vector<pair<int,int>> &fro,vector<int> &Can,map<int,int> &rconvert){
    bool deg_is1 = false;
    //元のグラフの経由頂点の1つでも次数が2未満であれば
    for(auto v:setx){
        if(g[v].size() < 2) deg_is1 = true;
    }
    //deg_istrueがtrueとなり
    //trueの場合
    if(deg_is1 == true){
        ...
        for(auto a:layer[i]){
            ...
            for(int b=0;b<=1;b++){
                ...
                if(b==0){
                    ...
                    //経由頂点の次数が2より小さいことが確定する場面
                    if(setx.count(v2)==true){
                        int count = deg1[v2];
                        for (auto e: g[v2]) {
                            if(e>i) count++;
                        }
                ...
                //経由地点が存在する前はs→tパスが成立していればそのパスは正しいパスであった
                //しかしながら今回の場合はs→tパスが必ず成り立つとは限らない
                //今回はs→tパスが成り立って,かつ経由頂点全てがs→tに含まれていればよい(つまり,経由頂点すべての次数が2であればよい)
                bool keiyu = true;
                for(auto v:setx){
                    if(deg1[v] != 2) keiyu = false;
                }
...
int main(){
    ...
    for(int i = 0;i<100;i++){ // 1つのデータに対して100回の施行を行っている
        set<int> stV;//G2はst及び経由頂点の辺集合
        ...
        stV.insert(s);
        stV.insert(t);
        set<int> setx;//経由頂点の集合
        for(int j=0;j<xn;j++){
            int come_x;
            do {
                int x = std::rand() % Can.size();
                come_x = x;
            } while (stV.count(come_x)); // 重複を避ける
            setx.insert(come_x);
            stV.insert(come_x);
        }
        cout << "s is " <<  rconvert[s] << " || " << "t is " << rconvert[t] << " || ";
        cout << "vn is ";
        for(auto v : setx){
            cout << rconvert[v] << " ";
        }
        cout << " || ";
        int paths = ZDD(V,E,s,t,setx,G,g,fro,Can,rconvert);
        cout << "path is" << paths << endl;
        path += paths;
    }
    // 終了時間を記録
